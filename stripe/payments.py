...
def __stripe_payment_method_pretty(payment_obj):
    ...
    if "object" in payment_obj:
        payment_method_id = None
        try:
            payment_method_obj = PaymentMethod.objects.get(payment_method_info__payment_method_type="source", 
                payment_method_info__payment_method_id=payment_obj["id"])
        except PaymentMethod.DoesNotExist:
            pass
        else:
            payment_method_id = payment_method_obj.id
        if payment_obj["object"] == "bank_account":
            return {
                "id": payment_method_id,
                "type": "bank_account",
...
def stripe_create_charge(**kwargs):
    ...
    if auto_charge:
        ...
        #d_args['off_session'] = off_session
        if is_save_payment:
            d_args['setup_future_usage'] = "on_session"
        d_args['statement_descriptor'] = description
        if metadata != False:
            d_args['metadata'] = metadata
        response = _stripe_api_call(stripe.PaymentIntent.create, **d_args)
...
def stripe_ach_create_charge(**kwargs):
    ...
    if backend_token == None and payment_method_id == None:
        raise Exception("Please pass a backend_token or a payment_method_id")
    ...
    if backend_token != None:
        ...
        try:
            payment_method = PaymentMethod.objects.get(merchant_id=merchant_id, unique_id=unique_id, id=payment_method_id)
        except PaymentMethod.DoesNotExist:
            return False, {"reason": "payment_method_not_exist"}
        else:
            d_args['source'] = payment_method.payment_method_info["payment_method_id"]
    response = _stripe_api_call(stripe.Charge.create, **d_args)
    if backend_token != None:
        if not is_save_payment:
            if d_args.get("stripe_account", None) == None:
            ...
            d_payment_method = {"payment_method_id": source_response['resource']['id'], "payment_method_type": "source"}
